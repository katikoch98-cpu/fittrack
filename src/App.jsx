...
const EX_PRESETS = ["Kniebeuge","Beinpresse","Beinbeuger","Beinstrecker","Hip Thrust","Ausfallschritte","Wadenheben","Kreuzheben","Bankdrücken","Schrägbankdrücken","Schulterdrücken","Klimmzüge","Rudern","Bizepscurl","Trizepsdrücken","Seitheben","Frontheben","Butterfly","Latzug","Plank","Crunch","Sit-ups","Romanian Deadlift","Hackenschmidt"];

const CARDIO_SPORTS = [
  { id:"jogging",    label:"Jogging",       emoji:"🏃", hasKm:true  },
  { id:"cycling",    label:"Fahrradfahren", emoji:"🚴", hasKm:true  },
  { id:"hiking",     label:"Wandern",       emoji:"🥾", hasKm:true  },
  { id:"volleyball", label:"Volleyball",    emoji:"🏐", hasKm:false },
  { id:"swimming",   label:"Schwimmen",     emoji:"🏊", hasKm:false },
  { id:"football",   label:"Fußball",       emoji:"⚽", hasKm:false },
  { id:"basketball", label:"Basketball",    emoji:"🏀", hasKm:false },
  { id:"tennis",     label:"Tennis",        emoji:"🎾", hasKm:false },
  { id:"yoga",       label:"Yoga",          emoji:"🧘", hasKm:false },
  { id:"boxing",     label:"Boxen",         emoji:"🥊", hasKm:false },
  { id:"skiing",     label:"Skifahren",     emoji:"⛷️", hasKm:true  },
  { id:"rowing",     label:"Rudern",        emoji:"🚣", hasKm:false },
  { id:"dancing",    label:"Tanzen",        emoji:"💃", hasKm:false },
  { id:"other",      label:"Sonstiges",     emoji:"🏅", hasKm:false },
];
...
const IcoPlay = ({s=18}) => <svg viewBox="0 0 24 24" fill="currentColor" stroke="none" style={{width:s,height:s}}><polygon points="5 3 19 12 5 21 5 3"/></svg>;
const IcoPause = ({s=18}) => <svg viewBox="0 0 24 24" fill="currentColor" stroke="none" style={{width:s,height:s}}><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>;
const IcoStop = ({s=16}) => <svg viewBox="0 0 24 24" fill="currentColor" stroke="none" style={{width:s,height:s}}><rect x="4" y="4" width="16" height="16" rx="2"/></svg>;
...
const Dot = ({color,dashed}) => <span style={{width:9,height:9,borderRadius:"50%",background:dashed?"transparent":color,border:dashed?`2px dashed ${color}`:"none",display:"inline-block",flexShrink:0}}/>;

// ─── Repeat helpers ──────────────────────────────────────────────
const defaultRepeat = () => ({freq:"none",endType:"forever",endDate:"",endCount:"10"});
const getRepeatFreq = (r) => (r&&typeof r==="object")?r.freq:(r||"none");
const repeatLabel = (r) => {
  const freq = getRepeatFreq(r);
  if(freq==="none") return "Einmalig";
  const base = freq==="weekly"?"Wöchentlich":"Alle 2 Wochen";
    ...
    <div style={{background:C.bg,borderRadius:12,padding:12,display:"flex",flexDirection:"column",gap:8}}>
      {[["none","Einmalig","Kein Wiederholungstermin"],["weekly","Wöchentlich",`Jeden ${wday}`],["biweekly","Alle 2 Wochen",`Jeden 2. ${wday}`]].map(([v,l,sub])=>(
        <button key={v} onClick={()=>set("freq",v)} style={{display:"flex",alignItems:"center",gap:10,background:r.freq===v?C.accL:"transparent",border:`1.5px solid ${r.freq===v?C.acc:"transparent"}`,borderRadius:10,padding:"10px 12px",cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
          ...
          <div><div style={{fontSize:14,fontWeight:r.freq===v?700:500,color:r.freq===v?C.acc:C.text}}>{l}</div><div style={{fontSize:11,color:C.light}}>{sub}</div></div>
        </button>
      ))}
      {r.freq!=="none"&&(
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:12}}>
          ...
          {r.endType==="date"&&<input type="date" value={r.endDate} min={date||todayISO()} onChange={e=>set("endDate",e.target.value)} style={inp({fontSize:14,marginTop:6})}/>}
          {r.endType==="count"&&(
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:6}}>
              <input type="number" min="1" max="104" value={r.endCount} onChange={e=>set("endCount",e.target.value)} style={inp({width:80,textAlign:"center",fontSize:16,fontWeight:700,padding:"8px"})}/>
...
// ─── Workout Timer ───────────────────────────────────────────────
function WorkoutTimer({onFinish}) {
  ...
  return (
    <div style={{background:"#1a1d2e",borderRadius:16,padding:"14px 16px",marginBottom:14,display:"flex",alignItems:"center",gap:12}}>
      <div style={{flex:1}}>
        <div style={{fontSize:11,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".07em",marginBottom:2}}>⏱ Workout-Zeit</div>
        <div style={{fontSize:28,fontWeight:800,color:"#fff",letterSpacing:"-.5px",fontVariantNumeric:"tabular-nums"}}>{fmtTimer(secs)}</div>
      </div>
      <button onClick={()=>setRunning(r=>!r)} style={{width:40,height:40,borderRadius:"50%",background:running?"#374151":"#10b981",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",flexShrink:0}}>
        {running?<IcoPause s={16}/>:<IcoPlay s={14}/>}
      </button>
      <button onClick={()=>onFinish(secs)} style={{width:40,height:40,borderRadius:"50%",background:"#ef4444",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",flexShrink:0}}>
        <IcoStop s={14}/>
      </button>
    </div>
  );
...
// ─── Last weight helper ──────────────────────────────────────────
function getLastWeights(sessions,name) {
...
function ExerciseBuilder({exercises,onChange,accentColor,sessions}) {
  ...
  const updEx = (id,name) => {
    ...
    onChange(exercises.map(e=>{
      if(e.id!==id) return e;
      const sets=lastW?e.sets.map((s,i)=>({...s,weight:lastW[i]!==undefined?lastW[i]:(lastW[lastW.length-1]||"")})):e.sets;
      return {...e,name,sets};
    }));
  };
  const addSet = (id) => onChange(exercises.map(e=>{
    if(e.id!==id) return e;
    const last=e.sets[e.sets.length-1]||{reps:"10",weight:""};
  ...
  const updSet = (id,si,f,v) => onChange(exercises.map(e=>e.id===id?{...e,sets:e.sets.map((s,i)=>i===si?{...s,[f]:v}:s)}:e));
  return (
    <div>
      {exercises.map((ex,ei)=>{
        ...
        return (
          <div key={ex.id} style={{...card(),border:`1px solid ${accentColor}22`,marginBottom:10}}>
            ...
            {lastW&&<div style={{background:accentColor+"0d",border:`1px solid ${accentColor}22`,borderRadius:8,padding:"6px 10px",marginBottom:8,fontSize:12,color:accentColor,fontWeight:600}}>💡 Letztes Mal: {lastW.map((w,i)=>`Set ${i+1}: ${w||"–"}kg`).join(" · ")}</div>}
...
function GymForm({initial,mode,onSave,onCancel,sessions,workoutTemplates,onSaveTemplate}) {
  const [form,setForm] = useState(()=>({
    ...
    label:initial.label||"",
    exercises:initial.exercises?.map(e=>({...e,id:e.id||uid(),sets:e.sets.map(s=>({...s}))})) || [],
  ...
  const loadTemplate = (t) => {
    u("label",t.label);
    u("exercises",t.exercises.map(e=>({...e,id:uid(),sets:e.sets.map(s=>({...s}))})));
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px",WebkitOverflowScrolling:"touch"}}>
        ...
        {/* Timer – only in log mode */}
        {mode==="log"&&<WorkoutTimer onFinish={(s)=>u("duration",s)}/>}
        {mode==="log"&&form.duration>0&&(
          <div style={{background:C.greenL,border:`1px solid ${C.green}33`,borderRadius:10,padding:"8px 14px",marginBottom:14,fontSize:13,color:C.green,fontWeight:700}}>
            ✓ Workout-Zeit erfasst: {fmtTimer(form.duration)}
          </div>
        )}
        ...
        {/* Template picker – shown in BOTH plan and log mode */}
            ...
            {showTplPicker&&(
              <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,marginTop:6,overflow:"hidden"}}>
                {workoutTemplates.map(t=>(
                  <button key={t.id} onClick={()=>loadTemplate(t)} style={{width:"100%",background:"transparent",border:"none",borderBottom:`1px solid ${C.border}`,padding:"12px 14px",textAlign:"left",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:10}}>
                    <span style={{fontSize:20}}>{t.emoji||"💪"}</span>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:C.text}}>{t.label}</div>
                      <div style={{fontSize:12,color:C.sub}}>{t.exercises.length} Übungen · {t.exercises.map(e=>e.name).filter(Boolean).slice(0,3).join(", ")}</div>
                    </div>
                  </button>
                ))}
              </div>
        ...
        {form.exercises.length>0&&onSaveTemplate&&(
          <div style={{marginBottom:14,marginTop:4}}>
            {!showSaveTpl
              ?<button onClick={()=>{setSaveTplName(form.label);setShowSaveTpl(true);}} style={{width:"100%",background:C.tplL,border:`1px dashed ${C.tpl}`,borderRadius:10,padding:"10px",color:C.tpl,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:7,fontFamily:"inherit"}}><IcoStar s={14}/> Als Vorlage speichern</button>
...
function CardioForm({initial,mode,onSave,onCancel}) {
  const [form,setForm] = useState(()=>({
    ...
    date:initial.date||todayISO(),time:initial.time||"",
    sport:initial.sport||"jogging",minutes:initial.minutes||"",
    km:initial.km||"",note:initial.note||"",
    repeat:initial.repeat&&typeof initial.repeat==="object"?initial.repeat:defaultRepeat(),
  }));
  const u = (k,v) => setForm(f=>({...f,[k]:v}));
  const sport = cardioInfo(form.sport);
  const valid = form.sport && form.minutes;
  const pace = sport.hasKm&&form.km&&form.minutes?calcPace(parseFloat(form.km),parseInt(form.minutes)*60):null;
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px 10px",borderBottom:`1px solid ${C.border}`,background:C.surface,flexShrink:0}}>
        <button onClick={onCancel} style={iBtn({background:C.bg,border:`1px solid ${C.border}`,padding:"8px 12px",gap:5,color:C.sub})}><IcoX/><span style={{fontSize:13,fontFamily:"inherit"}}>Abbrechen</span></button>
      ...
      <div style={{flex:1,overflowY:"auto",padding:16,WebkitOverflowScrolling:"touch"}}>
        ...
        <Fld label="Dauer (Minuten)"><input type="number" min="1" placeholder="45" value={form.minutes} onChange={e=>u("minutes",e.target.value)} style={inp()}/></Fld>
        {sport.hasKm&&(
        ...
        <Fld label="Notiz (optional)"><input placeholder="Route, Gefühl…" value={form.note} onChange={e=>u("note",e.target.value)} style={inp()}/></Fld>
        {mode==="plan"&&<Fld label="Wiederholung"><RepeatPicker value={form.repeat} onChange={v=>u("repeat",v)} date={form.date}/></Fld>}
        <div style={{height:30}}/>
      </div>
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"#11182766",zIndex:400,display:"flex",alignItems:"flex-end"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:C.surface,borderRadius:"20px 20px 0 0",width:"100%",paddingBottom:"env(safe-area-inset-bottom,16px)"}}>
        ...
        <div style={{padding:"8px 16px 16px"}}>
          ...
          <Fld label="Neues Datum"><input type="date" value={selectedDate} onChange={e=>setSelectedDate(e.target.value)} style={inp()}/></Fld>
...
function PlanRow({plan,onStart,onEdit,onDelete,onMove}) {
  ...
  const sport=!isGym?cardioInfo(plan.sport):null;
  const totalSets=isGym?plan.exercises?.reduce((a,e)=>a+e.sets.length,0):0;
  const freq=getRepeatFreq(plan.repeat);
  return (
    <div style={{...card(),borderLeft:`3px solid ${overdue?C.red:color}`,marginBottom:8,padding:"12px 14px"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:36,height:36,borderRadius:10,background:isGym?C.gymL:C.cardioL,color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:!isGym?20:14}}>
          {isGym?<IcoDumbbell s={18}/>:sport.emoji}
        </div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:2}}>{isGym?(plan.label||"Gym-Einheit"):`${sport.label}${plan.km?` · ${plan.km}km`:""}`}</div>
          <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:5}}>
            ...
            {freq!=="none"&&<span style={{display:"flex",alignItems:"center",gap:3,fontSize:11,background:C.accL,color:C.acc,borderRadius:6,padding:"2px 7px",fontWeight:600}}><IcoRepeat/> {repeatLabel(plan.repeat)}</span>}
        ...
        <div style={{display:"flex",gap:3,flexShrink:0}}>
          <button onClick={onMove} style={iBtn({padding:5})}><IcoMove/></button>
...
function SessionRow({session,expanded,onToggle,onDelete}) {
  ...
  const color=isGym?C.gym:C.cardio;
  const sport=!isGym?cardioInfo(session.sport):null;
  ...
  return (
    <div style={card()}>
      <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={onToggle}>
        <div style={{width:40,height:40,borderRadius:11,background:isGym?C.gymL:C.cardioL,color,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:!isGym?22:14}}>
          {isGym?<IcoDumbbell/>:sport.emoji}
        </div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:15,fontWeight:700,color:C.text,marginBottom:2}}>{isGym?session.label:`${sport.label}${session.km?` · ${session.km}km`:""}`}</div>
          <div style={{fontSize:12,color:C.sub}}>
            {fmtDate(session.date)} &nbsp;·&nbsp;
            {isGym
              ?`${session.exercises?.length||0} Übg. · ${totalSets} Sets · ${totalReps} Wdh.${session.duration?` · ${fmtTimer(session.duration)}`:""}`
              :`${session.minutes} min${session.km?` · ${calcPace(session.km,session.minutes*60)}`:""}`
            }
          </div>
        </div>
        ...
        <div style={{borderTop:`1px solid ${C.border}`,marginTop:12,paddingTop:12}}>
          {isGym?(
              ...
              {session.exercises?.map(ex=>(
                <div key={ex.id} style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                    {ex.sets.map((s,i)=><span key={i} style={{background:C.gymL,border:`1px solid ${C.gymB}`,color:C.gym,borderRadius:7,padding:"3px 9px",fontSize:12,fontWeight:600}}>{i+1}: {s.reps}×{s.weight||"–"}kg</span>)}
                  </div>
                </div>
              ))}
            </>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
              {[["Sportart",sport.emoji+" "+sport.label],["Dauer",session.minutes+" min"],...(session.km?[["Distanz",session.km+" km"],["Pace",calcPace(session.km,session.minutes*60)]]:[]),(session.note?[["Notiz",session.note]]:[])].flat(0).filter(Array.isArray).map(([l,v])=>(
          ...
          <button onClick={onDelete} style={{background:C.redL,border:`1px solid ${C.red}33`,borderRadius:9,color:C.red,padding:"8px 14px",fontSize:12,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}><IcoTrash/> Löschen</button>
        </div>
      )}
    </div>
  );
      ...
      <div style={{background:C.surface,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column",paddingBottom:"env(safe-area-inset-bottom,16px)"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:16,WebkitOverflowScrolling:"touch"}}>
                  ...
                  <div key={p.id} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:11,padding:"10px 12px",marginBottom:7}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      ...
                      <div style={{display:"flex",gap:4}}>
                        <button onClick={()=>onMovePlan(p)} style={iBtn({padding:5})}><IcoMove/></button>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            <button onClick={()=>onAddPlan("gym")} style={{background:C.gymL,border:`1.5px solid ${C.gymB}`,borderRadius:12,padding:"14px 10px",color:C.gym,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6,fontFamily:"inherit"}}><IcoDumbbell s={22}/>Gym planen</button>
            <button onClick={()=>onAddPlan("cardio")} style={{background:C.cardioL,border:`1.5px solid ${C.cardioB}`,borderRadius:12,padding:"14px 10px",color:C.cardio,fontSize:13,fontWeight:700,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6,fontFamily:"inherit"}}><IcoCardio s={22}/>Cardio planen</button>
          </div>
          <SecTitle>⚡ Direkt eintragen</SecTitle>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <button onClick={()=>{onLogDirect("gym");onClose();}} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 10px",color:C.sub,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5,fontFamily:"inherit"}}><IcoDumbbell s={20}/>Gym eintragen</button>
            <button onClick={()=>{onLogDirect("cardio");onClose();}} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 10px",color:C.sub,fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:5,fontFamily:"inherit"}}><IcoCardio s={20}/>Cardio eintragen</button>
          </div>
          <div style={{height:20}}/>
        </div>
      </div>
...
function TemplatesTab({templates,onDelete,onUse}) {
  return (
    <div style={{padding:"16px 14px"}}>
      <h2 style={{margin:"0 0 14px",fontSize:20,fontWeight:800}}>Vorlagen</h2>
      {templates.length===0?(
        <div style={{textAlign:"center",padding:"52px 0",color:C.light}}>
          ...
          <div style={{fontSize:13,marginTop:4,color:C.light}}>Speichere ein Workout als Vorlage beim Eintragen</div>
        </div>
      ):templates.map(t=>(
...
export default function App() {
  ...
  // ── Expand recurring plans correctly with endDate/endCount ──
  const allPlans = (() => {
    const result = [];
    // expand up to 1 year ahead so all months show correctly
    ...
    data.plans.forEach(p=>{
      ...
      if(freq==="weekly"||freq==="biweekly"){
        ...
        let count = 0;
        const maxCount = r.endType==="count"?parseInt(r.endCount||10):9999;
        const maxDate = r.endType==="date"&&r.endDate?r.endDate:futureISO;
        for(let i=0;i<104;i++){
          dt.setDate(dt.getDate()+step);
          const iso = dt.toISOString().slice(0,10);
          if(iso>maxDate||iso>futureISO) break;
          count++;
  ...
  // plans by date – only future/today dates for the calendar
  const plansByDate={};
  ...
  const todayPlans = (plansByDate[t]||[]).filter(p=>!sessionsByDate[t]?.some(s=>s.type===p.type));
  ...
  const savePlan = (form) => {
    ...
    persist({...data,plans});setScreen(null);setDayModal(null);
  };
  const deletePlan = (id) => {
  ...
  const movePlan = (plan,newDate) => {
    ...
    const plans=data.plans.map(p=>p.id===realId?{...p,date:newDate,repeat:defaultRepeat()}:p);
    persist({...data,plans});setMovingPlan(null);setDayModal(null);
  };
  ...
  const saveSession = (form) => {
    ...
    persist({...data,sessions:[session,...data.sessions],plans});setScreen(null);
  };
  ...
  return (
    <div style={{height:"100dvh",display:"flex",flexDirection:"column",background:C.bg,fontFamily:"'DM Sans',system-ui,sans-serif",overflow:"hidden",maxWidth:430,margin:"0 auto",paddingTop:"env(safe-area-inset-top,0px)"}}>
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:"calc(72px + env(safe-area-inset-bottom,16px))"}}>
          ...
          <div style={{padding:"16px 14px"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
              <button onClick={()=>setScreen({type:"log",form:{type:"gym",date:t}})} style={{background:C.gymL,border:`1.5px solid ${C.gymB}`,borderRadius:14,padding:"16px 12px",cursor:"pointer",textAlign:"left",display:"flex",flexDirection:"column",gap:6,fontFamily:"inherit"}}>
                ...
                <div style={{fontSize:11,color:C.sub}}>Sets, Reps & Timer</div>
              </button>
            ...
            <div style={{display:"flex",gap:12,marginBottom:10,flexWrap:"wrap"}}>
              {[[C.gym,"Kraft"],[C.cardio,"Cardio"]].map(([c,l])=>(
                <span key={l} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:C.sub}}>
                  <Dot color={c}/>{l} erledigt &nbsp;<Dot color={c} dashed/>{l} geplant
                </span>
              ))}
            </div>
        ...
        {tab==="history"&&(
          <div style={{padding:"16px 14px"}}>
            ...
            {filtered.length===0?<div style={{textAlign:"center",color:C.light,padding:"52px 0"}}>Keine Einträge.</div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:C.surface,borderTop:`1px solid ${C.border}`,display:"flex",paddingBottom:"env(safe-area-inset-bottom,0px)",zIndex:100,boxShadow:"0 -2px 16px #0000000d"}}>
        {[{id:"home",label:"Home",icon:<IcoHome/>},{id:"calendar",label:"Kalender",icon:<IcoCal/>},{id:"templates",label:"Vorlagen",icon:<IcoTemplates/>},{id:"history",label:"Verlauf",icon:<IcoHistory/>}].map(({id,label,icon})=>(
      ...
      {dayModal&&(
        <DayModal dateISO={dayModal} sessions={sessionsByDate[dayModal]||[]} plans={plansByDate[dayModal]||[]}
          ...
          onDeletePlan={deletePlan} onStartPlan={startPlan}
          onLogDirect={(type)=>setScreen({type:"log",form:{type,date:dayModal}})}
          onMovePlan={(p)=>{setDayModal(null);setMovingPlan(p);}}
        />
      )}
      {movingPlan&&<MovePlanModal plan={movingPlan} onMove={(d)=>movePlan(movingPlan,d)} onClose={()=>setMovingPlan(null)}/>}
    </div>
  );
}
